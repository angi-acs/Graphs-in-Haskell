...
splitNode node targets = extend f
    where
        f = \n -> if n == node then g else Node n
        g = foldl (\acc x -> Overlay (Node x) acc) Empty targets
