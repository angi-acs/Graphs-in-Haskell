...
mapSingle f = helper []
    where
        helper lst [] = []
        helper lst (x:xs)
            | null lst = (f x : xs) : helper (lst ++ [x]) xs
            | otherwise = (lst ++ [f x] ++ xs) : helper (lst ++ [x]) xs
...
partitions (x:xs) = map ([x]:) (partitions xs) ++
                    [lst | p <- partitions xs, lst <- mapSingle (x:) p]
...
isModule :: Ord a
         ...
         -> Bool
isModule set graph
    | S.size (helper outNeighbors) == 1 && S.size (helper inNeighbors) == 1 = True
    | otherwise = False
        where
            helper f = S.map (\x -> S.filter (`S.notMember` set) (f x graph)) set
...
isModularPartition :: Ord a
                   ...
                   -> Graph a
                   -> Bool
isModularPartition partition graph = all (== True) (S.map (`isModule` graph) partition)
...
maximalModularPartition partitions graph = minimumBy (compare `on` S.size) 
                                        (filter (\x -> isModularPartition x graph && (S.size x > 1)) partitions)
