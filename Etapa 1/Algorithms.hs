...
search f node graph = helper [] [node]
    where
        helper visited [] = reverse visited
        helper visited (x:xs) 
            | x `elem` visited = helper visited xs
            | otherwise = helper (x:visited) (f xs $ S.toList(outNeighbors x graph))
...
bfs = search f
        where
            f l1 l2 = l1 ++ l2 -- imbina listele conform unei cozi
{-
    *** TODO ***
...
dfs = search f
        where
            f l1 l2 = l2 ++ l1 -- imbina listele conform unei stive
...
countIntermediate from to graph
    | any (== True) [isNothing $ elemIndex n f | n <- [to, from], f <- [bfs', dfs']] = Nothing
    | otherwise = Just(x, y)
        where
            bfs' = bfs from graph
            x = fromJust(elemIndex to bfs') - fromJust(elemIndex from bfs') - 1
            dfs' = dfs from graph
            y = fromJust(elemIndex to dfs') - fromJust(elemIndex from dfs') - 1
-- daca un nod nu apare (in oricare dintre parcurgeri), returneaza Nothing
-- altfel, se face diferenta dintre pozitiile pe care apar nodurile in parcurgere