...
outNeighbors node graph = S.map snd $ S.filter (\n -> fst n == node) $ edges graph
...
removeNode node graph = fromComponents (S.toList $ S.delete node $ nodes graph) 
                                       (S.toList $ S.filter (not . memberPair node) $ edges graph)
                        where
                            memberPair elem' tuple = (fst tuple == elem') || (snd tuple == elem')
...
splitNode old news graph = removeNode old $ addNewComponents old news graph

addNewComponents old news graph = 
    fromComponents (newComponents nodes news graph) (newComponents edges (newEdges old news graph) graph)
    where
        newComponents f lst graph = S.toList $ foldl (\acc x -> S.insert x acc) (f graph) lst
        newEdges old news graph = [(x,y) | x <- S.toList(inNeighbors old graph), y <- news] ++ 
                                  [(x,y) | x <- news, y <- S.toList(outNeighbors old graph)]
-- newComponents reprezinta o functie abstracta care adauga, dupa caz, noduri sau muchii in graf
...
mergeNodes :: Ord a
           ...
           -> StandardGraph a  -- graful existent
           -> StandardGraph a  -- graful obținut
mergeNodes prop node graph = foldl (\acc x -> splitNode x [node] acc)
                                   graph  
                                   [x | x <- S.toList (nodes graph), prop x == True]
-- am folosit splitNode invers :), adica splitNode avea ca parametri old si news, 
-- iar mergeNodes apeleaza split pe o lista de olds si un singur new